...
public class ApplicationListener implements ServletContextListener {
	...
	private void initializeDatabase() {
		...
		Logger.getLogger("login").log(Level.INFO, "Creating databse from SQL script");
		try {
			Connection connection = DataSourceFactory.getDataSource().getConnection();
			int result = ij.runScript(connection, this.getClass().getResourceAsStream("export.sql"), "UTF-8", System.out, "UTF-8");
			if (result == 0) {
				Logger.getLogger("login").log(Level.INFO, "Database succesfully created");
			} else {
				Logger.getLogger("login").log(Level.SEVERE, "Errors creating database");
			}
		} catch (UnsupportedEncodingException | SQLException e) {
			Logger.getLogger("login").log(Level.SEVERE, null, e);
		}
